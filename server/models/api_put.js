...
export default (ctx) => {
    ...
    let add_terms = (post_id) => {
        //新增多条tag——不考虑重复
        let insert_tags = (names) => {
                ...
                if (names[0] == '404') {
                    ...
                    return query(sql_1).then(data1 => {
                        if (data1.affectedRows > 0) {
                            let values2 = null;
                            for (let n = 0; n < data1.affectedRows; n++) {
                                if (values2 == null) {
                        ...
                        if (data2.affectedRows > 0) {
                            let insert_ids = [];
                            for (let z = 0; z < data2.affectedRows; z++) {
        ...
        //更新term的count
        let update_term_count = (term_taxonomy_id) => {
            return new Promise((resolve, reject) => {
                let old_terms = "`term_taxonomy_id` = " + term_taxonomy_id[0];
                for (let n = 1; n < term_taxonomy_id.length; n++) {
        ...
        let category_ids = ctx.request.body.category;
        if (category_ids != '' && category_ids != null) {
            category_ids = category_ids.split(',');
        } else {
            category_ids = ['1'];
        }
        let category_sql = "`bm_terms`.`term_id` = " + category_ids[0];
        for (let n = 1; n < category_ids.length; n++) {
            category_sql += " OR `bm_terms`.`term_id` = " + category_ids[n];
        }

        let tags_name = ctx.request.body.post_tag;
        let tags_sql = '';
        if (tags_name != '' && tags_name != null) {
            tags_name = tags_name.split(',');
            for (let o = 0; o < tags_name.length; o++) {
            ...
            if (terms.length > 0) {
                ...
                return Promise.all([insert_tags(new_tags_name), update_term_count(term_taxonomy_id)]).then(terms_status => {
                    ...
                    if ((terms_status[0].status == 'ok' || terms_status[0].status == 'insert_ok') && terms_status[1] == 'ok') {
                        let add_terms = "(" + post_id + "," + term_taxonomy_id[0] + ",0)";
                        for (let n = 1; n < term_taxonomy_id.length; n++) {
                            add_terms += ",(" + post_id + "," + term_taxonomy_id[n] + ",0)";
                        }
                        if (terms_status[0].status == 'insert_ok') {
                            for (let q = 0; q < terms_status[0].insert_ids.length; q++) {
        ...
        //     let check_post_tag = (data, tag) => {
        //         return new Promise(function (resolve, reject) {
        //             let tags = tag.split(',');
        //             let tags_term_taxonomy_id = [];
        //             let new_tag_names = [];
        ...
        //
        //         let add_terms = null;
        //         for (let n = 0; n < back_terms.length; n++) {
        //             if (add_terms == null) {
    ...
    let array_sql = underscore.pairs(add_post_objects);
    let string_sql = '';
    for (let n = 0; n < array_sql.length; n++) {
...
//新增term
export let module_put_terms = (ctx) => {
